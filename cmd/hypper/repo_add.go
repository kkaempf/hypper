...
func newRepoAddCmd(out io.Writer) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			o.name = args[0]
			o.url = args[1]
			o.repoFile = settings.HelmSettings.RepositoryConfig
			o.repoCache = settings.HelmSettings.RepositoryCache
...
func (o *repoAddOptions) run(out io.Writer) error {
	...
	r, err := repo.NewChartRepository(&c, getter.All(settings.HelmSettings))
	if err != nil {
		return err
	}
